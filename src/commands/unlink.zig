...
pub fn execute(io: std.Io, allocator: std.mem.Allocator, args: UnlinkArgs, verbose: bool) !void {
    ...
    if (git.get_worktree_paths(io, allocator, target_dir)) |worktree_paths| {
        defer {
    ...
    if (args.path) |rel_path| {
        ...
        for (repos.items) |repo| {
            ...
            try revert_file(io, allocator, repo, rel_path);
        }
    ...
    for (repos.items) |repo| {
        ...
        try revert_link_directory(io, allocator, link_dir, repo, "");
        ...
        for (cfg.remove_patterns) |pattern| {
            try restore_removed_pattern(io, allocator, repo, pattern);
        }
    }
...
fn revert_link_directory(
    ...
    while (try iter.next(io)) |entry| {
        ...
        if (entry.kind == .directory) {
            try revert_link_directory(io, allocator, link_base, repo_base, new_rel);
        } else {
            try revert_file(io, allocator, repo_base, new_rel);
        }
    }
}
...
fn revert_file(
    ...
    const tracked = git.is_tracked(io, allocator, repo_path, rel_path) catch false;

    if (tracked) {
        // Clear the shadow flag, then delete our symlink and check out the
        // original file so the working tree matches HEAD again.
        git.no_skip_worktree(io, allocator, repo_path, rel_path) catch |err| {
        ...
        _ = git.restore_file(io, allocator, repo_path, rel_path) catch |err| {
        ...
        remove_from_local_exclude(io, allocator, repo_path, rel_path) catch |err| {
...
/// restore each one.
fn restore_removed_pattern(
    io: std.Io,
    allocator: std.mem.Allocator,
    repo_path: []const u8,
    pattern: []const u8,
) !void {
    const tracked_files = git.list_tracked_files(io, allocator, repo_path) catch |err| {
    ...
    for (tracked_files) |rel| {
        if (!fs_utils.match_glob(rel, pattern)) continue;

        git.no_skip_worktree(io, allocator, repo_path, rel) catch |err| {
            log.warn("could not clear skip-worktree on {s}: {}", .{ rel, err });
        };
        if (git.restore_file(io, allocator, repo_path, rel) catch false) {
            log.info("undeleted: {s}", .{rel});
        }
    }
}

fn remove_from_local_exclude(
    io: std.Io,
    allocator: std.mem.Allocator,
    repo_path: []const u8,
    rel_path: []const u8,
